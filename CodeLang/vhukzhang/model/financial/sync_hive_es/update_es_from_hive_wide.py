...
def handle(rows):
    ...
    for row in rows:
        ...
        # class_ids_is_null 字段
        # province city zone class_ids
        # 18494
        # if (
        #     rowDict["class_ids"]
        #     and rowDict["province"]
        #     and rowDict["city"]
        #     and rowDict["zone"]
        # ):
        #     rowDict["class_ids_is_null"] = 1
        # 18521 对齐
        if rowDict["class_ids"] and rowDict["province"]:
            rowDict["class_ids_is_null"] = 1
        num += 1
...
def main():
    print("start")
    ds = sys.argv[1]
    tableName = "sh2_urlsafe.t_fd_urlsafe_ngf_company_wide_table"
    ...
    hqlStr = f"SELECT {selectFields} FROM {tableName} WHERE ds={ds} AND class_ids is not NULL {LIMIT}"
    df = SPARK.sql(hqlStr)
    df.show(10)
    startTime = time.time()
